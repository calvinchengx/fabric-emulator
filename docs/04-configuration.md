...
| `FABRIC_ARM_URL` | `-arm-url` | *(none)* | arm-emulator origin. When set, `Microsoft.Fabric/capacities` created over ARM appear on `GET /v1/capacities`. Empty keeps the seeded local capacity only — standalone fabric-cicd still works. Opt-in: do not set this until the arm-emulator you point at serves `GET /_family/capacities`. |
