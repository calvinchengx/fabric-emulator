...
| `FABRIC_ARM_URL` | `-arm-url` | *(none)* | arm-emulator origin. When set, `Microsoft.Fabric/capacities` created over ARM appear on `GET /v1/capacities`. Empty keeps the seeded local capacity only — standalone fabric-cicd still works. Opt-in: the family compose in azure-emulators sets this; this repo's compose does not, so a throwaway fabric stack still works without ARM. |
