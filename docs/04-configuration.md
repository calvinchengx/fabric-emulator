...
| `FABRIC_AKV_VAULT_HOST` | — | *(none)* | One extra `host:port` an `AzureKeyVaultReference` may resolve against, besides Azure's own `*.vault.azure.net` and the sovereign equivalents. The compose file sets `keyvault-emulator:8444`. **This is a security boundary:** resolving a reference sends a vault-audience bearer token to the host the connection names, so anything unlisted is refused — otherwise a hostile `vaultURI` exfiltrates the token, and (pointed at a real tenant) a real Azure one. Azure constrains the URI the same way. |
