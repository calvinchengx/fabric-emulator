...
| **Compute permissions — semantic model** (TMSL `roles` / `tablePermissions`, OLS) | ❌ Not modelled, and today **silently**: `semanticmodel.Model` has no `roles` field, so a deployed model's role block is dropped by the JSON decode and DAX then evaluates unfiltered. The XMLA `TMSCHEMA_ROLES` / `TMSCHEMA_TABLE_PERMISSIONS` rowsets are named but answer empty. Refusing a model that carries roles is the honest increment before building one. |
