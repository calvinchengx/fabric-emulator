...
| **Compute permissions — semantic model** (TMSL `roles` / `tablePermissions`, OLS) | ⚠️ **Parsed, and refused rather than applied.** Both parsers used to drop roles, so DAX evaluated unfiltered for the principals a role restricts. Roles are now read from TMSL and TMDL, and a principal without Write on the model — the only principals roles apply to — is refused on every row-returning path (REST `executeQueries`, the XMLA loader, the portal runner) with a reason naming the roles. Write holders read as before, since the product exempts them. Evaluating the filters and hiding secured objects is staged in [58](58-semantic-model-roles.md). |
