...
| **Governance (OpenMetadata)** | real OpenMetadata 1.13.2 (Postgres) + delta-rs | the optional `governance` profile boots, `govern-ingest` catalogs workspace→lakehouse→Delta table, and OM's API returns the table with columns mapped from the real Delta log | `e2e/governance/run.py` (CI `governance`, Linux) |
