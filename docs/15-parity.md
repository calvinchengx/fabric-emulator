...
| Spark session / statement via the **Livy API** | **Native termination** (`--spark-agent-url`): the emulator implements the Livy REST session/statement contract itself and drives a Spark statement-executor agent — a real client's PySpark statements are computed by **real Spark**, and a session is a persistent REPL (state survives across statements), `e2e/livy`. No Apache Livy server (it's retired). Alternatively `--spark-livy-url` reverse-proxies an external Livy server. Batches (`spark-submit`) are a follow-up | 🟢 Real (interactive) / 🟠 batch |
