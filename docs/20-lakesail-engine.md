...
and batch-writes (announced; no checkpoint). Console stays on the engine
on Sail. Native kafka source and sink are the Eventstream wrap
([51-eventstream-kafka.md](51-eventstream-kafka.md)): Sail consumes or
produces through kafka-python (or the emulator for `eventstream.*`) and
runs `foreachBatch` in the agent; the JVM overlay uses OSS
