...
| Structured streaming, `OPTIMIZE`/`VACUUM`, Java/Scala UDFs | ⚠️ streaming **sinks** (delta / parquet / memory) are one announced micro-batch on the Livy path (`limit(n).collect()` + batch write; no checkpoint). OSS `format("kafka")` + bootstrap/subscribe and the Eventstream notebook API are the same class of wrap (driver/emulator consume, Kafka schema, bytes on Sail). `OPTIMIZE`/`VACUUM` via delta-rs. **Java/Scala UDFs and checkpointed streaming are the JVM overlay** | executable probes + engine-matrix |
...
- **Streaming sinks on the Livy path are one micro-batch**, not a checkpointed
  ...
  on the engine. Native `format("kafka")` + `kafka.bootstrap.servers` +
  `subscribe` consumes on the driver and `createDataFrame`s Kafka-schema rows
  into Sail ([51-eventstream-kafka.md](51-eventstream-kafka.md)). The Fabric
  ...
  UDFs all work), some catalog calls missing (`cacheTable`, `refreshTable`, …).
...
and batch-writes (announced; no checkpoint). Console and native kafka stay
on the engine on Sail. The Fabric Eventstream notebook API is the same wrap
class on both engines plus an opt-in Kafka broker
([51-eventstream-kafka.md](51-eventstream-kafka.md)): Sail consumes through
the emulator and runs `foreachBatch` in the agent; the JVM overlay uses OSS
`spark-sql-kafka`. Native checkpointed streaming is the JVM overlay, which
