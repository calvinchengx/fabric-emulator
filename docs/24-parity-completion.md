...
| ~~**Runtime divergences** (Environment items, Files mount, `input_file_name()` in SQL, inline pipeline jobs)~~ ✅ | Four analogs the code stated rather than hid, and all four are closed: Environments reach the session (#67, with `e2e/environment` and a negative control); the Files mount does write-back, refreshes at every statement, and refuses a second lakehouse rather than switching; `input_file_name()` in SQL is a lexer rewrite onto the tagged shadow view; and a pipeline job POST returns 202 for the client to poll (`TestPipelineJobOutlivesItsPOST`) instead of executing inline. **This row asked for the last two of those for months after [37](37-runtime-fidelity-gaps.md) marked both DONE** — the third stale row in this file, and the one that bought `scripts/check_plan_freshness.py`, which now fails the build when a row outlives the sub-plan it summarises. What remains under 37 is `2c′`, the agent-per-session pool, open by decision rather than backlog: revisit only if concurrent multi-lakehouse sessions become real. Scoped in [37-runtime-fidelity-gaps.md](37-runtime-fidelity-gaps.md) | — |
