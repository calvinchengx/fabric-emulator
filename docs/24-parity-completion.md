...
| **Runtime divergences** (Environment items, Files mount, `input_file_name()` in SQL, inline pipeline jobs) | Four analogs the code states rather than hides. Environment items are the cheapest: the parse and the installer both exist and are simply not wired together. Pipelines are the last job type that does not use the repo's own async pattern. Scoped in [37-runtime-fidelity-gaps.md](37-runtime-fidelity-gaps.md), which also corrects the Environments row of [parity.md](parity.md): it claimed packages were provisioned and Spark config applied, and neither happened, so the row is now 🟡 | XS–M each |
...
| Gap | Notes | Size |
|---|---|---|
| XMLA / ADOMD.NET (SemPy's transport) | Documented protocol (MS-XMLA / MS-SSAS); no harder than TDS was. A CI oracle DOES exist — `e2e/xmla` runs ADOMD.NET on Linux against a host we name, weekly, and asserts its first-call contract. Gated on cost and real demand, not feasibility. Scoped in [32-xmla-plan.md](32-xmla-plan.md), which names the demand (semantic-link-labs). That experiment RAN: the client now reaches XMLA and its connect path is measured, each gate costing one thing (one `clusterResolve` field, one `ExecuteResponse`, one echoed caps header). **The `L` stays**, deliberately — connect is the cheap half, and the estimate is dominated by `xmla-rs:rowset` serialisation plus the `Discover` metadata subset, neither of which any client has yet been asked to exercise. The evaluator itself is NOT in the estimate: `internal/semanticmodel` already runs DAX for `executeQueries`, so this is a transport | L |
| Full DAX | The bounded engine works; this is open-ended surface growth, not a blocker. An ORACLE may now exist — pbix-mcp ships per-function goldens captured from Desktop's own engine, which would make growth evidence-driven rather than doc-reading. Unverified; [33-pbix-tooling.md](33-pbix-tooling.md) Phase 0 | L, open-ended |
