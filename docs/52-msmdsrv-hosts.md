...
BLANK/`<=0` and `LOG` base `1`/`<=0` error), then `LN` (argument must be > 0; BLANK
coerces to 0 and errors), then `SIGN` (BLANK stays BLANK; `SIGN(0)` is 0), then `ASIN` / `ATAN` (BLANK stays BLANK; `ASIN` outside `[-1, 1]` errors), then `PI` / `SIN` / `COS` / `TAN` (`COS(BLANK())` is 1; `SIN`/`TAN` BLANK stays BLANK; `TAN` of a right angle errors), then `DEGREES` / `RADIANS` (BLANK stays BLANK). Captured on a UTM Windows 11 ARM guest + Desktop.
