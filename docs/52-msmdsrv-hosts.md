...
`EXP(BLANK())` is 1), then `SIGN` (BLANK stays BLANK; `SIGN(0)` is 0), then `ASIN` / `ATAN` (BLANK stays BLANK; `ASIN` outside `[-1, 1]` errors). Captured on a UTM Windows 11 ARM guest + Desktop.
