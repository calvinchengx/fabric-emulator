...
**Status: two of Fabric's documented shortcut behaviours are built — a OneLake
shortcut under `Tables/` reads as a table on the endpoint, and in user identity
mode a caller needs access at the source as well as at the consumer. The rest are
...
| In user identity mode, the caller is evaluated against the **source's** OneLake security, including its row and column rules | **Partly.** Table-level access at the source is enforced. The source's column and row narrowing is **not** applied to the consumer's read: a caller allowed the table at the source but narrowed there reads it whole through the consumer unless the consumer's own roles narrow it | — |
...
- **Owners are not refused.** A workspace Admin or Member is `db_owner`, and
...
- **Only the table-level check is the source's.** See the row above: this stage
  does not translate the source's roles into the consumer's database.
...
The per-caller refusal is the grant's `ShortcutTables` and `DeniedTables`
(`oneLakeGrant`), applied by `tds.SyncShortcutAccess` after the caller's role
memberships: `DENY SELECT` for a refused table, `REVOKE` (which removes a `DENY`)
for every other shortcut table. It runs after reflection, which drops and
recreates a table and its permissions with it.
