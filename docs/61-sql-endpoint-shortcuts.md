...
| In user identity mode, the caller is evaluated against the **source's** OneLake security, including its row and column rules; "when enforcement cannot clearly validate access, the system applies the most restrictive outcome" | **Built.** The source's column allow-list is a per-principal `DENY SELECT` on each column it withholds, and its row filters are synced as roles of their own and ANDed into the table's row policy with the consumer's. A read is narrowed by both sides, and the intersection wins. A Contributor is not column-narrowed, as OneLake security does not narrow one; a Contributor in a source role that filters is filtered, as row-level security "is enforced for all users". A source role's row filter naming a column the source table no longer has fails the whole sync on the consumer's endpoint, the same as one of the consumer's own roles ([docs/60](60-sql-endpoint-access-modes.md#documented-rules-and-the-witness-for-each)) | `TestAShortcutTableIsNarrowedByTheSourcesColumnsAndRows`, `TestARowLevelSecurityRoleAtAShortcutSourceNamingAMissingColumnFailsTheSync` |
