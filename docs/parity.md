...
| List **pagination** (`continuationToken`) | Opt-in via `?maxPageSize=N`: the page carries an opaque `continuationToken` and an **absolute** `continuationUri` (`https://host/v1/…?continuationToken=…`), matching the shape Fabric's List Items reference returns. Witnessed by Microsoft's **`fab` CLI** paging a workspace end to end — the token round-trips through its own HTTP stack, and the pages are asserted complete: no item twice, none missed, terminal page tokenless. **Boundary:** real Fabric pages on its own schedule, so a client that ignores the token still passes here | 🟢 Real (contract + real-client witness) |
