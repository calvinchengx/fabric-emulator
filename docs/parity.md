...
| Remaining admin-portal APIs (capacity admin, tenant-wide item listing) | — | 🔴 Not implemented |
...
| Java/Scala UDFs, `spark.jars` | **Impossible on Sail by construction** — it is a Rust engine with no JVM, so this can never be fixed upstream. The **JVM overlay** is the answer (Delta JVM classes verified present) | 🔴 default / 🟠 JVM overlay |
