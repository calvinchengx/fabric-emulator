...
| Environments | Run binding resolves the item's `requirements.txt`, Spark properties and JAR declarations, errors on a missing or wrong-typed Environment, and reports the result on the run as `environment`. **Nothing applies it**: no package is installed, no Spark config reaches the session, and a JAR-bearing run is not refused — the parsed `PythonPackages`/`SparkConfig`/`JARs` have no consumer outside the parser. The working substitute is a wheel directory bind-mounted at `/opt/wheels` and installed at agent startup, which is an analog of the item, not the item. Scoped in [37-runtime-fidelity-gaps.md](37-runtime-fidelity-gaps.md) | 🟡 Emulated (resolved and reported, not applied) |
...
| **REST connector** — `RestSource` in a Copy | Makes the **real HTTP request** and commits the rows: `url` (or the linked service's `url` + dataset's `relativeUrl`), `requestMethod` GET/POST, `requestBody`, `additionalHeaders` and `httpRequestTimeout`, all expression-resolved. The JSON response is shaped into a Delta table in the sink lakehouse — `translator.collectionReference` picks the record array, and a response holding exactly one array needs no hint. `Accept` is forced to `application/json` as Fabric does; a non-2xx fails the copy. Nested fields have no column shape and are reported by name in `skippedColumns` rather than dropped. Bounded at 8 MiB and 1M rows, refused not truncated. **Single page only**: `paginationRules` are **refused**, because reading page one of fifty and reporting success is the fabricated result this exists to remove (R2 — [40-rest-connector-plan.md](40-rest-connector-plan.md)). Auth is Anonymous + `additionalHeaders`; the emulator models no connections, so any other `authenticationType` fails by name — a Web activity can fetch a token and pass it as an expression, which is how BMC Helix's `AR-JWT` scheme works here | 🟢 Real (single page) |
| External-connector leaves (Salesforce, ServiceNow, …) | **Stubbed success** — reached in `dependsOn` order with inputs resolved, but nothing executes: each needs a vendor SDK and credentials the emulator has neither of. The **REST connector above is the working route** to any of them that expose a REST API — and to BMC Helix, which real Fabric has no connector for at all | 🟡 Emulated |
