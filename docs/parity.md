...
| `notebookutils.notebook.runMultiple` — notebook DAG orchestration | Runs a DAG of notebooks in dependency order, accepting either a bare list or Fabric's `{activities, timeoutInSeconds, concurrency}` shape. A failed activity's dependents are reported `Skipped` with the reason rather than run; a cycle or a dependency naming an activity outside the DAG is refused. **Sequential** — `concurrency` is accepted and ignored, because the Spark sidecar is one session, so parallel submission would queue anyway while making failure attribution worse. Remaining gaps (`exitVal` always `""`, per-cell timeout applied whole-notebook, `useRootDefaultLakehouse` ignored) are scoped in [39-run-multiple-parity-plan.md](39-run-multiple-parity-plan.md) | 🟡 Real, sequential |
...
| External-connector leaves (Salesforce, ServiceNow, …) | **Stubbed success** — reached in `dependsOn` order with inputs resolved, but nothing executes: each needs a vendor SDK and credentials the emulator has neither of | 🟡 Emulated |
