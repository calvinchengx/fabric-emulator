...
**Nested columns (`struct`/`array`/`map`) no longer corrupt the row.**
[**Correction — the omission described here does not work in this release.**
They come back present, typed `varchar`, holding NULL. See Known issues below;
fixed in v0.16.1.] They never worked: the reader assigned Parquet *leaf* values
by position, so one nested column took another field's leaf and displaced
**every column after it**, dropping the real values, with nothing raised. That
displacement **is** fixed here and confirmed on the released image — the columns
around a nested one now carry their own values. Fabric does not represent these
types at all, so the intent was to drop the column entirely; what shipped drops
it in the reader and re-adds it as NULL one stage later. The omitted names are
logged — except that this release cannot log them either, for the same reason.
...
- **The nested-column omission above does not actually work in this release.**
  ...
  [16-warehouse-tds.md](https://github.com/calvinchengx/fabric-emulator/blob/main/docs/16-warehouse-tds.md).
