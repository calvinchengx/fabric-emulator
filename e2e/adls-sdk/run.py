#!/usr/bin/env python3
"""e2e A1: the real delta-rs engine (`deltalake`) writes and reads a Delta
...
runs the driver with dependencies from the locked `adls-sdk` uv group."""
...
WORK = os.path.join(tempfile.gettempdir(), "adls-sdk-e2e")
...
if not entra_bin:
    ...
    subprocess.run(
        ["go", "install", "github.com/calvinchengx/entra-emulator/cmd/entra-emulator@v0.2.0"],
...
try:
    ...
    log("running the Azure Blob SDK against the emulator")
