...
WORK = os.path.join(tempfile.gettempdir(), "delta-rs-e2e")
...
try:
    ...
    log(f"starting fabric-emulator on :{FABRIC_PORT} (plain HTTP for object_store)")
    start("fabric", [
        fabric_bin, "-addr", f"127.0.0.1:{FABRIC_PORT}",
        "-data-dir", os.path.join(WORK, "data"), "-disable-tls",
