#!/usr/bin/env python3
"""e2e: `pyapacheatlas`, a real third-party Apache Atlas v2 client, drives
fabric-emulator's Purview Data Map, authenticated by entra-emulator on the
Data Map's own `purview.azure.net` audience.

This exists to satisfy one stated condition. `docs/parity.md`'s Purview row is
graded 🟡 for exactly one reason — its evidence is 14 Go tests, and a 🟢 row in
this repo requires a REAL-CLIENT witness in CI. The Data Map is Apache Atlas v2
(the Azure spec's own TypeSpec says so), which is why an off-the-shelf Atlas
client is a legitimate witness rather than a client shaped like our code.

Scope is deliberately the type system and entities ONLY; see driver.py."""
...
WORK = os.path.join(tempfile.gettempdir(), "purview-datamap-e2e")
...
def wait_healthy(url, deadline=60):
    ...
    ctx.verify_mode = ssl.CERT_NONE  # self-signed harness certs (entra)
