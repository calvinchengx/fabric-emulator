...
| reading silver | needs `07_reflect.py` to copy every Delta table into SQL first | reads the Delta files directly — **no reflection step exists here** |
...
`07_compare.py` reads the `gold_summary.json` each example writes and reports
three things:
