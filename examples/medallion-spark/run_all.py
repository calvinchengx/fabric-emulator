...
example OWNS is where the two paths diverge, which is everything from gold on:
    ...
    ../medallion            here
    07_reflect.py           (nothing — Spark reads the Delta directly)
    08_gold.py    dbt-fabric      06_gold_spark.py   dbt-fabricspark
    …                             07_compare.py
...
have run before `07_compare.py` has two halves to compare.
...
Python REPL — so one agent serves both this example and 04_engine.py's notebook.
"""
import os
import pathlib
import subprocess
...
SHARED = [
    ("00_provision.py", "provision workspace + lakehouse"),
    ("01_secret.py", "store the source API key in Key Vault + AKV reference"),
    ...
    ("04_engine.py", "Spark executes the queued notebook run"),
    ("05_silver.py", "silver: dedupe, conform, quarantine"),
]

OWN = [
    ("06_gold_spark.py", "gold: dbt-fabricspark builds the star with Spark SQL"),
    ("07_compare.py", "compare both gold builds — equivalence, dialect, wall-clock"),
]
