...
func TestSpliceForward(t *testing.T) {
	...
	go func() { _ = spliceSession(clientA, backendA, nil, false, nil, "") }()
...
func TestSpliceForwardsReadOnlySelect(t *testing.T) {
	...
	defer clientA.Close()
	defer backendA.Close()
	go func() { _ = spliceSession(clientA, backendA, isWriteStatement, false, nil, "") }() // read-only surface
...
func TestSpliceBackendClosed(t *testing.T) {
	...
	go func() { done <- spliceSession(clientA, backendA, nil, false, nil, "") }()
	go func() { _ = WriteMessage(clientB, PktSQLBatch, batchMsg("SELECT 1")) }()
	select {
...
func TestSpliceRejectWriteError(t *testing.T) {
	...
	go func() { done <- spliceSession(clientA, backendA, isWriteStatement, false, nil, "") }()
