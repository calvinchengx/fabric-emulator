...
var strictCorpus = []struct {
	...
	{"FOR JSON in a scalar subquery", "select (select a from t for json path) as j", "for-json-subquery"},
	{"FOR JSON in a CTE", "with c as (select a from t for json path) select * from c", "for-json-subquery"},
	{"FOR JSON in a derived table", "select * from (select a from t for json auto) x", "for-json-subquery"},
	{"FOR JSON in an IN subquery", "select a from t where a in (select b from u for json auto)", "for-json-subquery"},
	{"FOR JSON two levels down", "select json_query((select a from t for json path))", "for-json-subquery"},
	{"FOR JSON in a subquery beside an outer FOR JSON", "select a, (select b from u where u.k = t.k for json path) as bs from t for json path", "for-json-subquery"},
	{"view name with a slash", "create view dbo.[a/b] as select 1 x", "object-name-character"},
	{"table name with a backslash", `create table [a\b] (x int)`, "object-name-character"},
	{"schema name with a slash", "create schema [a/b]", "object-name-character"},
	{"schema part of a table name", "create table [my/schema].t (x int)", "object-name-character"},
	{"SELECT INTO a name with a slash", "select 1 x into dbo.[a/b]", "object-name-character"},
	{"CTAS into a name with a slash", `create table dbo."a/b" as select 1 x`, "object-name-character"},
	{"ALTER VIEW with a slash", "alter view dbo.[a/b] as select 1 x", "object-name-character"},
	{"CREATE OR ALTER VIEW with a slash", "create or alter view [a/b] as select 1 x", "object-name-character"},
	...
	{"FOR JSON as the last operator", "select a from t for json path", ""},
	{"FOR JSON with options", "select a from t for json path, root('r'), include_null_values", ""},
	{"FOR JSON in a view body", "create view v as select a from t for json path", ""},
	{"FOR JSON after ORDER BY", "select a, b from t where c = 1 order by a for json auto", ""},
	{"FOR JSON after a JOIN", "select a from t join u on t.k = u.k for json path", ""},
	{"FOR JSON after an INSERT column list", "insert into j (doc) select a from t for json path", ""},
	{"FOR JSON after a subquery in its FROM", "select a from (select a from t) x for json path", ""},
	{"FOR JSON inside a string literal", "select 'select (select a from t for json path)' as s", ""},
	{"a column named with a slash", "create table t ([a/b] int, c varchar(10) default 'x/y')", ""},
	{"a column reference with a slash", "select [a/b] from t", ""},
	{"division", "select a/b, c\\d from t", ""},
	{"a name with a slash in a literal", "select 'create view [a/b] as select 1'", ""},
	{"an ordinary view name", "create view dbo.ok as select 1 x", ""},
	{"SELECT INTO an ordinary name", "select 1 x into dbo.ok", ""},
	{"INSERT INTO is not a creation", "insert into dbo.t select 1", ""},
...
func TestStrictCorpusCoversEveryFeature(t *testing.T) {
	...
	for _, want := range []string{
		...
		"for-json-subquery", "object-name-character",
	} {
		if !seen[want] {
