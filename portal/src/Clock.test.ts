...
describe('Clock', () => {
  ...
  it('shows the clock state and freezes it', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation((url: RequestInfo | URL, opts?: RequestInit) => {
    ...
    expect(sentBody(postCall)).toEqual({ freeze: true });
  });
  ...
  it('advances by the entered amount', async () => {
    ...
    const postCall = fetchMock.mock.calls.find(([, o]) => o?.method === 'POST')!;
    expect(sentBody(postCall)).toEqual({ advance: 601 });
  });

  it('surfaces API errors', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(errRes('boom', 500));
    render(Clock);
  ...
  it('unfreezes a frozen clock', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(res({ now: 1700000000, offset: 0, frozen: true }));
    render(Clock);
    await waitFor(() => expect(screen.getByText('frozen')).toBeInTheDocument());
    await fireEvent.click(screen.getByRole('button', { name: 'Unfreeze' }));
    await waitFor(() => expect(fetchMock.mock.calls.length).toBeGreaterThan(1));
    expect(sentBody(fetchMock.mock.calls.at(-1))).toEqual({ freeze: false });
  });

  it('resets the offset and unfreezes in one call', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(res({ now: 1700000000, offset: 90, frozen: false }));
    render(Clock);
    await waitFor(() => expect(screen.getByText('90s')).toBeInTheDocument());
    await fireEvent.click(screen.getByRole('button', { name: 'Reset' }));
    await waitFor(() => expect(fetchMock.mock.calls.length).toBeGreaterThan(1));
    expect(sentBody(fetchMock.mock.calls.at(-1))).toEqual({ offset: 0, freeze: false });
  });
  ...
  it('surfaces a failed mutation without losing the clock it is showing', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(() => {
      call += 1;
      return call === 1
        ? Promise.resolve(res({ now: 1700000000, offset: 0, frozen: false }))
        : Promise.resolve(errRes('clock is held', 409));
    });
    render(Clock);
    await waitFor(() => expect(screen.getByText('running')).toBeInTheDocument());
    await fireEvent.click(screen.getByRole('button', { name: 'Freeze' }));
